...
fs.readFile(path.join(__dirname, './index.html'), 'utf-8', (err, datastr) => {
    ...
    // 2.3 读取 HTML 文件成功后，调用对应的方法，拆解出 CSS、JS 和HTML文件
    resolveCSS(datastr)
    resolveJS(datastr)
    resolveHTML(datastr)
})
...
function resolveCSS(htmlStr) {
    ...
    // 3.4 将提取出来的CSS 样式，写入到index.css文件中
...
function resolveJS(htmlStr) {
    ...
    // 4.4 将提取出来的JS 样式，写入到index.js文件中
...
function resolveHTML(htmlStr) {
    // 5.1 使用字符串的 replace 方法，把内嵌的 < style > 和</style > 标签，替换为外联的 < link > 和 < script > 标签
    const newHTML = htmlStr
        .replace(regStyle, '<link rel="stylesheet" href="./index.css"/>')
        .replace(regScript, '<script src="./index.js"></script>')
    // 5.2 j将替换完成之后的 html 代码，写入到 indexedDB.html 文件中
